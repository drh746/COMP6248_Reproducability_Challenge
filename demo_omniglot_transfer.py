...
for i, alphabet in enumerate(omniglot_evaluation_alphabet_set):
    ...
    argv = '--gpuid %d --dataset Omniglot_eval_%s --model_type vgg --model_name VGGS --schedule 100 --epochs 150 --print_freq 0 --loss %s --out_dim %d --skip_eval --use_SPN' % (
    config.gpuid, alphabet, config.loss, config.num_cluster)
    argv = argv.split(' ')
