...
def get_args(argv, model_type, model_name, lo):
    ...
    parser.add_argument('--epochs', type=int, default=140, help="End epoch")
    ...
    parser.add_argument('--schedule', nargs="+", type=int, default=[60, 70],
...
if __name__ == '__main__':
    start = time.time()
    a, loss_ce = run(get_args(sys.argv[1:], "vgg", "VGG11", "CE"))
    end = time.time()
    print('CE time:', end - start)
    start = time.time()
    a, loss_kcl = run(get_args(sys.argv[1:], "vgg", "VGG11", "KCL"))
    end = time.time()
    print('KCL time:', end - start)
    start = time.time()
    a, loss_mcl = run(get_args(sys.argv[1:], "vgg", "VGG11", "MCL"))
    end = time.time()
    print('MCL:', end - start)
    ...
    # net = ['VGG8', 'VGG11', 'VGG16']
    # loss = ['CE','KCL','MCL']
    # for n in net:
    #     if n =="LeNet":
    #         a, loss_ce = run(get_args(sys.argv[1:],"lenet",n,'CE'))
    #         a, loss_kcl = run(get_args(sys.argv[1:],"lenet",n, "KCL"))
    #         a, loss_mcl = run(get_args(sys.argv[1:],"lenet",n, "MCL"))
    #         plt.figure(figsize=(8, 5))
    #         plt.plot(loss_ce)
    #         plt.plot(loss_kcl)
    #         plt.plot(loss_mcl)
    ...
    #         plt.show()
    #     else:
    #         a, loss_ce = run(get_args(sys.argv[1:], "vgg", n, 'CE'))
    #         a, loss_kcl = run(get_args(sys.argv[1:], "vgg", n, "KCL"))
    #         a, loss_mcl = run(get_args(sys.argv[1:], "vgg", n, "MCL"))
    #         plt.figure(figsize=(8, 5))
    #         plt.plot(loss_ce)
    #         plt.plot(loss_kcl)
    #         plt.plot(loss_mcl)
